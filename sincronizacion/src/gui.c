...
static GtkLabel *label_proc = NULL;
static GtkLabel *label_res = NULL;
static GtkLabel *label_act = NULL;
...
static void on_file_select(GtkButton *button, gpointer user_data) {
    ...
    GtkLabel *label = (GtkLabel *)g_object_get_data(G_OBJECT(button), "label");
...
void launch_gui(int argc, char *argv[]) {
    ...
    label_proc = GTK_LABEL(gtk_label_new("Archivo no seleccionado"));
    GtkWidget *btn_res = gtk_button_new_with_label("Seleccionar Recursos");
    label_res = GTK_LABEL(gtk_label_new("Archivo no seleccionado"));
    GtkWidget *btn_act = gtk_button_new_with_label("Seleccionar Acciones");
    label_act = GTK_LABEL(gtk_label_new("Archivo no seleccionado"));

    g_object_set_data(G_OBJECT(btn_proc), "target", &file_procesos);
    g_object_set_data(G_OBJECT(btn_proc), "label", label_proc);

    g_object_set_data(G_OBJECT(btn_res), "target", &file_recursos);
    g_object_set_data(G_OBJECT(btn_res), "label", label_res);

    g_object_set_data(G_OBJECT(btn_act), "target", &file_acciones);
    g_object_set_data(G_OBJECT(btn_act), "label", label_act);
    ...
    gtk_grid_attach(GTK_GRID(grid_files), GTK_WIDGET(label_proc), 1, 0, 1, 1);

    gtk_grid_attach(GTK_GRID(grid_files), btn_res, 0, 1, 1, 1);
    gtk_grid_attach(GTK_GRID(grid_files), GTK_WIDGET(label_res), 1, 1, 1, 1);

    gtk_grid_attach(GTK_GRID(grid_files), btn_act, 0, 2, 1, 1);
    gtk_grid_attach(GTK_GRID(grid_files), GTK_WIDGET(label_act), 1, 2, 1, 1);
